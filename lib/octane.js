...
async function runOctaneLinks({ testNames, config, session, delayMs, dryRun }) {
  ...
  const notFound = [];
  const octaneClientVersion = session['octane-client-version'] || '26.2.8.91';
  ...
  const baseHeaders = {
    ...
    'octane-client-version':  octaneClientVersion,
  ...
  for (let i = 0; i < testNames.length; i++) {
    ...
    if (!dryRun) {
      try {
        ...
        if (res.status === 401) {
          console.log('');
          log('error', '401 Unauthorized — authentication has expired. Refresh curl/session input and retry.');
